    ...
    def button_clicked(self, e):
        ...
        # 四則演算子
            ...
            self.result.value = str(
                self.calculate(self.operand1, float(str(self.result.value)), self.operator)
            )
            self.operator = data
        ...
        # ▼ 三角関数（度数法）＋指数関数 e^x ＋ 対数 ln, log10
        elif data in ("sin", "cos", "tan", "e^x", "ln", "log"):
            try:
                x = float(str(self.result.value))
...
def main(page: ft.Page):
    page.title = "Simple Calculator with Trig, Exp & Log"
    calc = CalculatorApp()
    page.add(calc)
