...
def save_forecasts_to_db(area_code: str, area_name: str, forecasts: list[dict]):
    ...
    forecasts の要素: {"date": "YYYY-MM-DD", "weather": "天気の文章"}
    """
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        for item in forecasts:
            cur.execute(
                ...
                INSERT OR REPLACE INTO forecasts
                    (area_code, area_name, forecast_date, weather)
                VALUES (?, ?, ?, ?);
                """,
                (area_code, area_name, item["date"], item["weather"]),
            )
        conn.commit()
...
def load_forecasts_from_db(area_code: str, date: str | None = None) -> list[dict]:
    ...
    return [{"date": r[0], "weather": r[1]} for r in rows]
...
def load_forecast_jma(area_code: str, max_days: int = 5) -> list[dict]:
    ...
    戻り値:
        [
          {"date": "2026-01-09", "weather": "晴れ　時々　くもり"},
          ...
        ]
    """
    ...
    for i, iso_dt in enumerate(time_defines[:max_days]):
        # "2026-01-09T00:00:00+09:00" → "2026-01-09"
...
def main(page: ft.Page):
    page.title = "天気予報アプリ（気象庁API + SQLite）"
    ...
    def on_select_area_jma(area_code: str, area_name: str):
        ...
        date_dropdown.value = None  # 「全日表示」に戻す
    ...
    # 右側パネル（タイトル + ステータス + 日付選択 + 予報カラム）
    right_panel = ft.Container(
        expand=True,
        padding=10,
        content=ft.Column(
            controls=[
                ft.Text("天気予報（DB利用版）", size=22, weight=ft.FontWeight.BOLD),
                status_text,
                date_dropdown,
                ft.Divider(),
                forecast_column,
